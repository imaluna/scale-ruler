import {getDataType, isObject, deepMerge, floor, getGridSize, getOffset} from './utils'
export default class ScaleRuler {
	...
	constructor(options) {
		...
		const opt = deepMerge(ScaleRuler.defaultOpt, options);
		...
		if (opt.proxyScaleKey) {
			document.addEventListener(
				'keydown',
				ScaleRuler.keydownEvent.bind(this)
			);
		}
		// 手指放大
	...
	checkLarge() {
		let {opt} = this;
		const {scale, canvasConfig, wrapConfig, showScrollBar, scrollConfig} = opt;
		const {translateX, translateY} = canvasConfig;
		// 画布加上两侧padding的宽度
